...
const TopicModelTree = (props: ITopicModelTree) => {
  ...
  const [chartGapX, setChartGapX] = useState<number>(150); // 设置图的x轴的上节点的间距
    ...
    if (selectedTopicId) {
      ...
      try {
        ...
        getTopicCluster(
          ...
          if (res.ok) {
            console.log('集群图数据', res.data, res.data.data);
  ...
  const drawTopicTree = (
    ...
    const tip = d3Tip()
      .attr("class", "d3-tip")
      .html(function (e, d, type="merged-g") {
        let htr = '';
        console.log(999, e, d, type);
        
        if(type === "merged-g"){
          if (d.data.type === "merged") return d.data.topic;
          let htr = `repo number: ${d.data.repoIds.length}<br />`;
          if (d.data.restRepoIdsLength) {
        ...
        // for (let word of d.data.words) {
        //   if(word['p'] > topicWordsThreshold){
        //     htr += `${word['word']}: ${word['word']}`
        //   }
        // }
        return htr;
      });
  ...
  function zoomed({transform}) {
    ...
    function update(source) {
      ...
      const nodeEnter = node
        ...
        //   setSelectedTopicId(d.data.id); // 设置当前选择的主题id，更新下方视图数据
        // })
      ...
      for (let i = 0; i < nodeEnter._groups[0].length; i++) {
        ...
        if (curDatum.data.type === "merged") {
          ...
          let outerRadius = 20, innerRadius = outerRadius - 3, circleR = 20;
          try {
            ...
            // 当前节点拥有的词，只在图中最多展示出前三个词的信息
            const words = curDatum.data.words.filter(e => e.p > topicWordsThreshold);
  ...
  const onChangeTopicModelMinTopicSize = (value) => {
          ...
          <div className="topic-tree-control">
            ...
            <div className="tree-layout-setting">
              ...
              <div className="icon-wrapper">
                <MinusOutlined />
                <Slider onChange={setChartGapX} value={chartGapX} min={50} max={230} step={10}/>
                <PlusOutlined />
              </div>
              {/* 设置单个节点的高度 */}
              ...
              <div className="icon-wrapper">
                <MinusOutlined />
                <Slider onChange={setChartGapY} value={chartGapY}  min={30} max={120} step={10}/>
                <PlusOutlined />
              </div>
            </div>
            <div className="tree-layout-setting">
